...
- Стек: TypeScript + Vite + React + shadcn/ui (Tailwind CSS + Radix UI); роутинг — react-router-dom; иконки — lucide-react; календарь — react-day-picker (обёртка `calendar` из shadcn/ui); даты — date-fns.
- API-клиент пишется вручную под каждый путь OpenAPI (генерация кода не применяется): единый модуль с функциями под owner/events/slots/bookings; базовый URL берётся из `VITE_API_BASE_URL` (в dev — Vite proxy на отдельно запущенный бэкенд).
- Для разработки и проверки интерфейса без запущенного бэкенда используется Stoplight Prism — эмулятор API по контракту: `npx @stoplight/prism-cli mock spec/dist/openapi.yaml` (мок-ответы по сгенерированному OpenAPI).
- Фронтенд — отдельная часть приложения: получает данные и выполняет действия только через API по контракту; корректно работает с отдельно запущенным бэкендом.
- Страница владельца (`/owners/:id`): профиль владельца, создание события (название, описание, длительность 15–480 мин), список событий, копирование ссылки на событие в буфер обмена.
- Страница события (`/owners/:id/events/:eventId`): название/описание/длительность, календарь на 14 дней, свободные слоты на выбранную дату, форма бронирования (имя, email), сообщение об успехе.
- Страница `/owners/:id/bookings`: список всех бронирований владельца.
- Ошибки API (400/404/409) отображаются пользователю; локальная валидация форм (email, длительность) до отправки запроса.
- Чек-лист готовности: создание события и копирование ссылки работают; календарь на 14 дней показывает только свободные слоты (даты вне окна недоступны); бронирование проходит и появляется в `/bookings`; ошибки API (400/404/409) отображаются пользователю; фронт собирается без ошибок (`make build`); интерфейс корректно работает с отдельно запущенным бэкендом и с Prism-эмулятором по контракту.
